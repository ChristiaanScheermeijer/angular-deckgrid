...
angular.module('akoenig.deckgrid').factory('Deckgrid', [
  ...
  function initialize($window, $log) {
    ...
    Deckgrid.prototype.$$createColumns = function $$createColumns() {
      ...
      if (this.$$scope.layout.columns === this.$$scope.columns.length) {
        if (this.$$modelCache.length < this.$$scope.model.length) {
          var clone = this.$$scope.model.slice();
          this.$$fillColumns(clone.splice(this.$$modelCache.length), this.$$modelCache.length);
        } else if (this.$$modelCache.length > this.$$scope.model.length) {
          angular.forEach(this.$$scope.columns, function (column) {
            var cardCount = column.length;
            while (cardCount--) {
              if (self.$$scope.model.indexOf(column[cardCount]) === -1) {
                column.splice(cardCount, 1);
              }
            }
          });
        }
        ...
        this.$$scope.columns = [];
        this.$$fillColumns(this.$$scope.model, 0);
      }

      this.$$modelCache = this.$$scope.model.slice();
    };

    /**
     * Fill columns with cards data
     *
     * @param cards
     * @param indexOffset if appending cards set the index offset
     */
    Deckgrid.prototype.$$fillColumns = function $$fillColumns(cards, indexOffset) {
      var self = this;

      for (var i = 0; i < this.$$scope.layout.columns; i++) {
        if (!self.$$scope.columns[i]) {
          self.$$scope.columns[i] = [];
        }
      }

      angular.forEach(cards, function (card, index) {
        var column = ((index + indexOffset || 0) % self.$$scope.layout.columns) | 0;
